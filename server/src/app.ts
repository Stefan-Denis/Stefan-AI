...
app.post('/videos/add', upload.array('videos'), async (req, res) => {
    try {
        const badFiles: Array<string> = []
        const promises = (req.files as Express.Multer.File[]).map(async (file: Express.Multer.File) => {
...
app.get('/themes', (req, res) => {
    const themes: themeFile = themeFile.getThemes()
...
app.post('/calculate-videos', (req, res) => {
    ...
    res.status(200).send({ maxCombinations: calculateAllCombinations(maxVideoUsageInput, videosPerArrayInput, files.length) as number })
})
